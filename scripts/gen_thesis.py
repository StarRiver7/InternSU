...
OUTPUT = r"F:\_Code\Ai\InternSU\docs\documents\InternSU_BYSJBG.docx"
os.makedirs(os.path.dirname(OUTPUT), exist_ok=True)
print("Generating thesis DOCX at:", OUTPUT)
...
def pp(text, cn=BT, en=BE, sz=12, bold=False, align=WD_ALIGN_PARAGRAPH.JUSTIFY, sb=0, sa=6, fi=0.74, color=None, ls=None):
...
def hd(text, lv):
    ...
    smap = {0:24, 1:18, 2:14, 3:10}; smap2 = {0:12, 1:10, 2:8, 3:6}
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER if lv == 0 else WD_ALIGN_PARAGRAPH.LEFT
    pf = p.paragraph_format
    pf.space_before = Pt(smap.get(lv, 12)); pf.space_after = Pt(smap2.get(lv, 8))
    pf.first_line_indent = Cm(0)
    r = p.add_run(text); font(r, HT, HE, sizes.get(lv, 12), lv <= 2)
...
def code(text):
    ...
    r = p.add_run(text); font(r, CF, CF, 9, False, RGBColor(0x33, 0x33, 0x33))
    return p
