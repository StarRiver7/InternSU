"""RAG 回答节点 —— 从检索上下文生成带引用的结构化回答。

【架构定位】
该节点是 RAG 子图的出口节点，位于 citation_node 之后、response_node 之前。
负责将检索到的文档片段组装为上下文，调用大模型生成最终回答，并附加来源引用。

【数据流】
  citation_node → rag_answer_node → response_node

【可信度驱动的回答策略】
根据 trust_level（由 citation_node 评估）选择不同的 Prompt 变体：
  - trust_level = "unreliable" → 拒绝编造，返回"未找到"
  - trust_level = "low" → 带提示回答，注明信息可能不完整
  - trust_level = "medium/high" → 正常引用回答

【Prompt 设计原则】
  1. 强制要求 LLM 使用 [来源N] 标注每个声明
  2. 引导 LLM 仔细阅读所有资料，避免遗漏
  3. 设置低温 (temperature=0.1) 保证回答稳定性，减少幻觉
  4. 以"收到老师～"开头保持人格化风格
"""
...
async def rag_answer_node(state: InternState) -> InternState:
    """生成 RAG 回答 —— 根据检索上下文和可信度等级调用 LLM。

    核心逻辑：
    1. 从 state 中提取 rag_context（格式化的检索上下文）和 trust_level
    2. 构建包含资料、规则、问题的 Prompt
    3. 调用 LLM 生成带引用标注的回答
    4. 将回答、来源列表写回 state

    Args:
        state: LangGraph InternState，关键字段：
            - user_message: 用户原始问题
            - rag_context: 格式化后的 RAG 上下文（由 citation_node 生成）
            - trust_level: 可信度等级 (high/medium/low/unreliable)
            - citations: 结构化引用列表
            - model_name: LLM 模型名称

    Returns:
        更新后的 InternState，关键字段：
        - rag_answer: RAG 生成的回答文本
    ...
    # ── 构建来源列表（供前端展示和引用标注）──────────────────────
    # 从 citation_node 输出的 citations 中提取来源信息
    ...
    for i, c in enumerate(citations):
        ...
        sources.append({
            "label": source_label,
            "text": content[:200],  # 截取前 200 字符作为摘要
...
## 你的回答"""
        ...
        # 调用 LLM 生成回答
        # temperature=0.1: 低温设置，减少幻觉和随机性，保证准确性优先
        # max_tokens=2048: 限制回答长度，避免过长上下文导致质量下降
...
async def _generate_no_results(query: str, model: str) -> str:
    ...
    当 RAG 检索无结果时，调用 LLM 生成一个诚实的"未找到"回复，
    而非硬编码的固定文本，保持回答的自然性和人格化风格。
...
def _finish_trace(state: InternState, message: str, t0: float) -> None:
    """完成追踪步骤，将最后一条 trace 更新为 completed 状态。

    Args:
        state: LangGraph 状态对象
        message: 最终消息
        t0: 起始时间戳（用于计算耗时 duration_ms）
    """
