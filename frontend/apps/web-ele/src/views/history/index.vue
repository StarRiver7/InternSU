...
const historyList = ref<HistoryItem[]>([
  {
    ...
    messages: [
      ...
      { id: 2, content: '人工智能（Artificial Intelligence，简称AI）是计算机科学的一个分支，旨在研究、开发用于模拟、延伸和扩展人的智能的理论、方法、技术及应用系统。', isUser: false },
      { id: 3, content: '人工智能有哪些应用场景？', isUser: true },
      { id: 4, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 5, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 6, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 7, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 8, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 9, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 10, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 11, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 12, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 13, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 14, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 15, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
      { id: 16, content: '人工智能的应用场景非常广泛，包括：自动驾驶、医疗诊断、金融风控、智能家居等。', isUser: false },
    ],
  },
  {
    id: 2,
    ...
    messages: [
      ...
      { id: 2, content: '在Python中，可以使用多种方法实现列表去重，包括使用set()或dict.fromkeys()等方法。', isUser: false },
    ],
  },
  {
    id: 3,
...
async function handleSendMessage() {
  ...
  // 添加用户消息（不带 traces）
  const history = historyList.value.find(item => item.id === selectedHistoryId.value);
  if (history) {
  ...
  if (history) {
    ...
    // 设置最终回复内容
    aiMessage.content = '收到你的消息了！这是小 SU 的智能回复。';
  }
...
const selectedHistory = computed(() => {
  ...
  return historyList.value.find(item => item.id === selectedHistoryId.value);
});
    ...
    <div class="flex-1 px-4 pb-4 pt-4 overflow-hidden -mt-4">
      <div class="w-full max-w-10xl mx-auto h-full flex gap-4 pt-4">
        ...
        <div class="w-80 h-full bg-background/5 border border-border backdrop-blur-lg rounded-2xl shadow-lg p-4 flex flex-col flex-shrink-0">
            ...
            <div v-for="[groupName, items] in groupedHistory" :key="groupName">
              ...
              <div class="space-y-2">
                <div v-for="item in items" :key="item.id"
                  class="group flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-all" :class="[
          ...
          <!-- Empty State -->
          <div v-if="historyList.length === 0" class="flex flex-col items-center justify-center py-20">
        ...
        <div class="flex-1 h-full flex min-w-0 pt-15 flex-col">
          <div v-if="selectedHistory" class="flex-1 bg-transparent rounded-2xl p-4 flex flex-col min-h-0">
            ...
            <div class="flex-1 overflow-y-auto space-y-4 messages-container mb-4">
              <div v-for="message in selectedHistory.messages" :key="message.id"
                ...
                <div v-if="message.isUser"
                  ...
                  :style="{ width: message.content.length < 20 ? 'auto' : 'fit-content', minWidth: '80px' }">
            ...
            <div class="flex-shrink-0">
              <div
                ...
                <textarea ref="textareaRef" v-model="inputText"
                  :placeholder="selectedHistory ? '输入消息...' : '选择历史记录后开始聊天...'"
                  :disabled="!selectedHistory || isLoading" @keydown="handleKeydown" @input="adjustHeight()" rows="1"
                ...
                <div class="flex items-center justify-between px-3 pb-3">
                  ...
                  <button @click="handleSendMessage" :disabled="!inputText.trim() || isLoading || !selectedHistory"
          ...
          <div v-else class="flex-1 bg-transparent rounded-2xl flex flex-col items-center justify-center">
            ...
            <p class="text-gray-500 text-sm">点击左侧历史记录查看详细消息</p>
          </div>
        </div>

        <!-- Right Side: Trace Panel -->
        <div
          class="w-80 h-80 bg-white/80 border border-gray-200 backdrop-blur-sm rounded-2xl p-4 flex flex-col flex-shrink-0"
          ...
          <!-- Trace Content -->
          <div v-if="selectedHistory" class="flex-1 overflow-y-auto space-y-3">
            <template v-for="message in selectedHistory.messages" :key="message.id">
              <div v-if="message.traces && message.traces.length > 0">
                <!-- Trace Items -->
                <div v-for="trace in message.traces" :key="trace.id"
                  class="p-3 bg-white rounded-lg border border-gray-100">
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-xs font-medium text-gray-700">{{ trace.step }}</span>
